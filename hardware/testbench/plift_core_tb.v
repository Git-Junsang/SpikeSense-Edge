// plift_core_tb.v — PLIF-T 뉴런 단독 검증
// 대상: plift_core.v (조합논리, 클럭 없음)
//
// Vivado: 시뮬 소스에 plift_core_tb.v + hardware/src/plift_core.v 추가,
//   top을 plift_core_tb로 두고 Run Behavioral Simulation → "run all".
//   외부 파일 의존성 없어 바로 실행 가능.
//
// iverilog:
...
module plift_core_tb;
    ...
    initial begin
        ...
        // TC1: 발화 + Soft Reset
        // beta=200(≈0.78): decayed=(200×100)>>8=78, mem_new=78+50=128 ≥ 50 → spike=1
        ...
        // TC3: 음수 막전위 누설
        // product=200×(−100)=−20000 → [23:8]=−79, mem_new=−79+5=−74 < 20 → spike=0
        ...
        // decayed=0, mem_new=0+30=30 ≥ 20 → spike=1, mem_out=30−20=10
        ...
        // TC5: beta=255 (최대 누설, ≈0.996)
        // product=255×200=51000 → [23:8]=199, mem_new=199 ≥ 30 → spike=1, mem_out=169
