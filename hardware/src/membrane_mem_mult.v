// membrane_mem_mult.v — 다중 트랙 막전위 메모리 (시분할용)
//
// 단일채널 membrane_mem.v의 다중 트랙 확장판.
//   - 트랙당 256엔트리 스트라이드(뉴런 0~161 사용, 나머지 미사용)
//     주소 = {track, neuron} 으로 곱셈 없이 연결해 타이밍 유리
//   - BRAM 추론을 위해 동기 읽기(1클럭 지연) + 동기 쓰기
//   - 비동기 리셋 없음. 버퍼 시작 막전위 초기화는 상위(mult_snn_top)에서
//     first_ts일 때 mem_old를 0으로 마스킹해 대체 (BRAM 클리어 불필요)
//
// 읽기 주소(rd_*)가 뉴런 처리 내내 안정적이라 1클럭 읽기지연이
// write-back 전에 자연히 해소된다 (별도 prefetch 제어 불필요).

`timescale 1ns/1ps

module membrane_mem_mult #(
