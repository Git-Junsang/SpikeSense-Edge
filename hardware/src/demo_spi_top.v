...
// 유일한 차이: 엔진을 mult_snn_top(Leaky Counter) 대신 demo_snn_top(세그먼트 단위 판정)으로 교체.
//   → LED[track] = 해당 트랙의 가장 최근 완료 세그먼트 정상/이상.
//     매 세그먼트마다 깔끔히 토글 (이상→정상→정상→이상 시연 가능).
//
// 인스턴스 이름(u_clkdiv / u_spi / u_mult)을 mult_spi_top과 동일하게 유지해
// 기존 nexys_a7_mult.xdc 의 계층 제약(생성클럭·ASYNC_REG)을 그대로 재사용한다.
// 합성 시 top module 을 demo_spi_top 으로 지정하고 같은 XDC를 쓰면 된다.
//
// 참고: mult_spi_top.v / dual_snn_top.v / snn_top.v 는 동결. 본 모듈은 신규.
