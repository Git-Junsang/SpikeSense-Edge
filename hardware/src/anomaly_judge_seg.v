...
//   기존 Leaky Counter는 감쇠 1/16384라 한 번 올라간 카운터가 세그먼트가
//   바뀌어도 거의 안 내려와(래치 성격), "이상→정상→정상→이상" 같은 매
//   세그먼트 토글 데모에 부적합하다.
//
// 본 모듈은 **트랙 버퍼(31타임스텝) 단위로 스파이크를 다수결**한다:
...
//
// 골든 hw_* 벡터에서 분리 마진이 큼(정상 29:2, 이상 0:31)이라 동점 없음.
// ※ anomaly_judge_mult.v / mult_* 는 동결. 본 모듈은 신규.
// ============================================================
