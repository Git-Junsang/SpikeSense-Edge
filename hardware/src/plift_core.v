// ============================================================
// plift_core.v — PLIF-T 뉴런 연산 코어
// ============================================================
// lif_core.v 확장 버전: β와 V_th를 런타임 입력으로 받음
// (뉴런별 학습 파라미터 지원)
//
// 연산 순서:
...
//   4) Soft Reset: mem_out = spike ? (mem_new - vth_ext) : mem_new
//
// beta: Q0.8 uint8  (0~255 → 0.0~0.996)
// vth : INT8 양의 정수 (범위 7~109, 16-bit zero-extend)
// 막전위: 16-bit signed
// 전체 조합논리 — 클럭 없음
// ============================================================
...
module plift_core #(
    ...
    // ① 누설: (beta × mem_in) >> 8
    // -------------------------------------------------------
    // {1'b0, beta}: 9-bit unsigned → $signed: 9-bit signed (항상 양수)
    // product 최대: 255 × 32767 = 8,355,585 < 2^23 → 24-bit 안전
    ...
    // -------------------------------------------------------
    // ③ 발화 판정 (vth: INT8 → 16-bit zero-extend, 항상 양수)
    // -------------------------------------------------------
