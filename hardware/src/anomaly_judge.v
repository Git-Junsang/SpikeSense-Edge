...
//
// 출력 뉴런(Out0, Out1)의 스파이크를 Leaky Counter로 누적, cnt_anomaly > cnt_normal이면
// anomaly_flag를 올린다. update 펄스(FSM done)마다 두 카운터를 갱신·비교.
//
// 감쇠: cnt = cnt - (cnt >> 14) + spike
//   decay = 1 - 1/16384 = 0.99994, 실효 윈도우 ≈ 16384 타임스텝 ≈ 1.02초(@16kHz)
//   시프트+뺄셈만 쓰므로 곱셈기 불필요.
...
module anomaly_judge #(
    ...
    // 감쇠량 계산 (조합논리): leak = cnt >> SHIFT_N, 새 값 = cnt - leak + spike
