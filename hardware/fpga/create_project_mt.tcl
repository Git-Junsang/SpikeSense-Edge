# ============================================================
# create_project_mt.tcl — SpikeSense-Edge Phase 6-2 (다중 트랙 시분할)
# ============================================================
# 대상 : Nexys A7-100T (xc7a100tcsg324-1), Top = mt_spi_top (50 MHz)
# 실행 :
#   (A) Windows cmd.exe — 프로젝트 생성만:
#         vivado -mode batch -source create_project_mt.tcl
#   (B) Windows cmd.exe — 합성·구현·비트스트림까지:
#         vivado -mode batch -source create_project_mt.tcl -tclargs run
#   (C) Vivado GUI Tcl Console:
#         source {Z:/.../SpikeSense-Edge/hardware/fpga/create_project_mt.tcl}
#         # 전체 흐름까지: source 앞 줄에 → set ::run_flow 1
# ------------------------------------------------------------
# dual용 create_project.tcl과 동일 구조. 차이: top=mt_spi_top,
# XDC=nexys_a7_mt.xdc(50MHz 생성클럭), bit=mt_spi_top.bit.
...
set proj_name  "vivado_spikehw_mt"
# ⚠️ 저장소 경로는 반드시 ASCII (한글 폴더면 합성 run이 무에러 크래시).
set proj_dir   "$repo_root/hardware/fpga/vivado_spikehw_mt"
set part       "xc7a100tcsg324-1"

set src_dir    "$repo_root/hardware/src"
set wt_dir     "$src_dir/weights"
set xdc_file   "$repo_root/hardware/constraints/nexys_a7_mt.xdc"
set tb_file    "$repo_root/hardware/testbench/tb_mt_snn_top.v"
...
# ---- RTL 소스 (hardware/src/*.v 전체) + Top=mt_spi_top ----
#   ⚠ 공백 포함 경로 → 각 파일을 [list $f]로 감싼다.
foreach f [glob "$src_dir/*.v"] {
    add_files -norecurse [list $f]
}
set_property top mt_spi_top [get_filesets sources_1]
...
set _xdc_obj [get_files -quiet *nexys_a7_mt.xdc]
if { $_xdc_obj ne "" } {
    set_property used_in_synthesis false $_xdc_obj
}

# ---- 시뮬레이션 (선택): tb_mt_snn_top ----
if { [file exists $tb_file] } {
    add_files -fileset sim_1 -norecurse [list $tb_file]
    set_property top tb_mt_snn_top [get_filesets sim_1]
}
...
if { $::run_flow } {
    ...
    set bit "$proj_dir/$proj_name.runs/impl_1/mt_spi_top.bit"
    puts "INFO: 비트스트림 → $bit"
}
